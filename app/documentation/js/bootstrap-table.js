...
!(function ($) {
    ...
    BootstrapTable.prototype.initContainer = function () {
        ...
        if (
            $.inArray('table-no-bordered', this.options.classes.split(' ')) !==
            -1
        ) {
        ...
        $.each(this.options.columns, function (i, columns) {
            ...
            $.each(columns, function (j, column) {
                ...
                if (typeof column.fieldIndex !== 'undefined') {
                    ...
                    that.header.clickToSelects[column.fieldIndex] =
                        column.clickToSelect
                    that.header.searchables[column.fieldIndex] =
                        column.searchable
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        html = [
            sprintf(
                '<div class="columns columns-%s pull-%s">',
                ...
                ? $.grep(this.data, function (item, i) {
                      for (var key in item) {
                          ...
                          var value = item[key],
                              column =
                                  that.columns[
                                      getFieldIndex(that.columns, key)
                                  ],
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        if (this.options.totalRows) {
            if (this.options.pageSize === this.options.formatAllRows()) {
                ...
                if (
                    pageLst.indexOf(
                        this.options.formatAllRows().toLowerCase()
                    ) > -1
                ) {
                    $allSelected = true
                }
        ...
        $.each(pageList, function (i, page) {
            if (
                ...
                if ($allSelected) {
                    active =
                        page === that.options.formatAllRows()
                            ? ' class="active"'
                            : ''
                } else {
                    active =
                        page === that.options.pageSize ? ' class="active"' : ''
                }
                pageNumber.push(
                    sprintf(
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            if (this.options.cardView) {
                html.push(
                    sprintf('<td colspan="%s">', this.header.fields.length)
                )
            }
    ...
    BootstrapTable.prototype.updateSelected = function () {
        ...
        this.$selectItem.each(function () {
            $(this)
                .parents('tr')
                [$(this).prop('checked') ? 'addClass' : 'removeClass'](
                    'selected'
                )
        })
    }
    ...
    BootstrapTable.prototype.fitFooter = function () {
        ...
        scrollWidth =
            elWidth > this.$tableBody.width() ? getScrollBarWidth() : 0
    ...
    BootstrapTable.prototype.toggleRow = function (index, isIdField, visible) {
        ...
        $(this.$body[0])
            ...
            .filter(
                sprintf(
                    isIdField ? '[data-uniqueid="%s"]' : '[data-index="%s"]',
                    index
                )
            )
            [visible ? 'show' : 'hide']()
